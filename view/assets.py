...
def obter_animacao_bau(classe, resultado, tamanho=(400, 200), fps=10):
    ...
    def ordenar_frames(nome):
        numeros = "".join(c for c in nome if c.isdigit())
        return int(numeros) if numeros else 0

    frames = []

    arquivos = sorted(os.listdir(base_path), key=ordenar_frames)
    ...
    if not frames:
        print(f"Nenhum frame encontrado em {base_path}")
        return None
